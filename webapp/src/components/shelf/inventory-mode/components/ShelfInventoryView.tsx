  ...
  return (
    <Stack spacing={1.5}>
      <Stack direction="row" spacing={1} useFlexGap sx={{ flexWrap: 'wrap' }}>
        <Chip label={`${shelves.length} shelf${shelves.length === 1 ? '' : 'ves'}`} />
        <Chip
          label={`${invalidProductIds.size} invalid placement${invalidProductIds.size === 1 ? '' : 's'}`}
          color={invalidProductIds.size > 0 ? 'warning' : 'default'}
          variant="outlined"
        />
      </Stack>
